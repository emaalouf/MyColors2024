...
class MagentoEncryptor
{
    ...
    // Laravel's Config for key management
    public function __construct()
    {
        $this->keys = preg_split('/\s+/', trim((string)Config::get('app.key')));
    ...
    public function getHash(string $password, $salt = false, int $version = self::HASH_VERSION_LATEST): string
    {
    ...
    {
        if (!in_array($version, [self::HASH_VERSION_MD5, self::HASH_VERSION_SHA256])) {
            throw new \InvalidArgumentException('Unknown hashing algorithm');
        }

        return hash($this->getHashAlgorithm($version), $data);
    }

    public function hash(string $data, int $version = self::HASH_VERSION_SHA256): string
    {
        if (empty($this->keys[$this->keyVersion])) {
            throw new RuntimeException('No key available');
        }

        return hash_hmac($this->getHashAlgorithm($version), $data, $this->keys[$this->keyVersion]);
    }
    ...
    {
        $agnosticArgonRegEx = '/^' . self::HASH_VERSION_ARGON2ID13_AGNOSTIC . '\_(?<seed>\d+)\_(?<ops>\d+)\_(?<mem>\d+)$/';
        ...
        try {
            ...
            foreach ($hashVersions as $hashVersion) {
                if (is_string($hashVersion) && preg_match($agnosticArgonRegEx, $hashVersion, $argonParams)) {
                    ...
                    $recreated = $this->generateSimpleHash($hashSalt . $recreated, $hashVersion);
                }
            }
        ...
        return hash_equals($recreated, $hash);
    }

    private function explodePasswordHash(string $hash): array
    {
        ...
        $explodedPassword[self::PASSWORD_VERSION] = explode(self::DELIMITER, $explodedPassword[self::PASSWORD_VERSION]);

        return $explodedPassword;
    }

    private function getArgonHash(string $data, int $seedBytes, int $opsLimit, int $memLimit, string $salt): string
    {
    ...
    private function getHashAlgorithm(int $version): string
    {
        return match ($version) {
            self::HASH_VERSION_MD5 => 'md5',
            self::HASH_VERSION_SHA256 => 'sha256',
            default => throw new \InvalidArgumentException('Unknown hashing algorithm'),
        };
    }
}
